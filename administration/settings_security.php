...
// inserted for update to new gReCaptcha v2
if(!IsSet($settings['recaptcha_type'])) {
	$result = dbquery("INSERT INTO ".DB_SETTINGS." (`settings_name`, `settings_value`) VALUES ('recaptcha_type', 'text');");
...
if (isset($_POST['savesettings'])) {
	...
	if ($_POST['captcha'] == "recaptcha" && ($_POST['recaptcha_public'] == "" || $_POST['recaptcha_private'] == "")) {
		...
		if (!$result) { $error = 1; }
		$result = dbquery("UPDATE ".DB_SETTINGS." SET settings_value='".stripinput($_POST['recaptcha_type'])."' WHERE settings_name='recaptcha_type'");
		if (!$result) { $error = 1; }
	}
...
echo "<span style='margin-top:7px; display:block;'>".$locale['699']."</span>";
echo "</div>";
echo "</td>\n";
echo "<td width='50%' class='tbl'>";
...
echo "<option value='light'".($settings['recaptcha_theme'] == "light" ? " selected='selected'" : "").">".$locale['697a']."</option>\n";
echo "<option value='dark'".($settings['recaptcha_theme'] == "dark" ? " selected='selected'" : "").">".$locale['697b']."</option>\n";
echo "</select><br />\n";
echo "<select name='recaptcha_type' size='1' class='textbox' style='margin-top:5px;'>\n";
echo "<option value='text'".($settings['recaptcha_type'] == "text" ? " selected='selected'" : "").">".$locale['699a']."</option>\n";
echo "<option value='audio'".($settings['recaptcha_type'] == "audio" ? " selected='selected'" : "").">".$locale['699b']."</option>\n";
echo "</select>\n";
echo "</td>\n";
echo "</tr>\n<tr>\n";
