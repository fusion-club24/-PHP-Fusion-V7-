...
if ($_REQUEST['stype'] == "news" || $_REQUEST['stype'] == "all") {
	...
	if ($fieldsvar) {
		$rows = dbcount("(news_id)", DB_NEWS, groupaccess('news_visibility')." AND ".$fieldsvar." AND (news_start='0'||news_start<=".time().") AND (news_end='0'||news_end>=".time().") ".($_REQUEST['datelimit'] != 0 ? " AND news_datestamp>=".(time() - $_REQUEST['datelimit']) : ""));
	} else {
		$rows = 0;
	}
	if ($rows != 0) {
		...
		$result = dbquery(
			...
			".($_REQUEST['datelimit'] != 0 ? " AND news_datestamp>=".(time() - $_REQUEST['datelimit']) : "")."
