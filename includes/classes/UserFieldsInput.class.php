...
class UserFieldsInput {
	...
	private function _verifyNewEmail() {
		...
		$mailbody = str_replace("[SALUTATION]", $salutation, $mailbody);
		$mailbody = str_replace("[USER_LOGIN]", $userdata['user_name'], $mailbody);		
	...
	private function _setEmailVerification() {
		...
		if(IsSet($settings['login_method']) && $settings['login_method'] != '1') { $message = str_replace("LOGIN_NAME", $this->_userName, $locale['u152']); }
		else { $message = str_replace("LOGIN_NAME", $this->_userEmail, $locale['u152']); }
		$message = str_replace("USER_NAME", $this->_userName, $message);
